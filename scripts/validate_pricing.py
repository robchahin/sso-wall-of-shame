...
def validate_vendor_file(filepath):
    ...
    if not base_pricing or not sso_pricing:
        warnings.append("Missing base_pricing or sso_pricing fields.")
        return True, warnings, errors # We might not want to fail hard if fields are just missing entirely depending on schema strictness, but let's warn.
